...
- Return a single JSON object matching the output structure defined in these instructions. The workflow passes the full object to the Knowledge Curator gate.
...
1. Call search_rd_knowledge with sessionId and a non-empty query built from document identifiers, compound names, trial IDs, endpoints, or dataset codes from the ingestion payload.
...
| `vectorsIndexed` | Count of embeddable chunks prepared for the Vector DB |
